...
object Lift {
  ...
  implicit def liftId[M[_]]: Lift[M, M] = {
    new Lift[M, M] {
      def lift[A](ba: M[A]): M[A] = ba
    }
  }

  implicit def liftLayer[M[_], Inner[_]](implicit layer: monad.Layer[M, Inner]): Lift[Inner, M] = {
    new Lift[Inner, M] {
  ...
  (implicit liftF1: Lift[B, M], liftF2: Lift[B2, B]): Lift[B2, M] = {
    new Lift[B2, M] {
